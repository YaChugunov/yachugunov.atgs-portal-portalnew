...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
$output = "";
// Дата повышения лимита с 300р до 400р
$limit_old    = "300.00";
$LIMITUP_DATE = "2025-02-17";
//
$reqLimit = mysqli_fetch_array(mysqlQuery_atgsdinner("SELECT day_limit FROM users WHERE id='{$_SESSION['id']}'"));
$limit    = $reqLimit['day_limit'];
    ...
    if (checkUserAuthorization($_SESSION['login'], $_SESSION['password']) == -1) {
        ...
        if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ! empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
            ...
            if ((checkServiceAccess('allservices') == 1 && checkServiceAccess('portalnew') == 1 && checkUserRestrictions($_SESSION['id'], 'portalnew', 2, 0) == 1) or (checkServiceAccess('allservices') == 1 && checkServiceAccess('portalnew') == 0 && checkIsItSuperadmin($_SESSION['id']) == 1) or (checkServiceAccess('allservices') == 0 && checkIsItSuperadmin($_SESSION['id']) == 1)) {
                ...
                $_QRY_1  = mysqlQuery_atgsdinner("SELECT `order_id`, SUM(item_total) as sum, `order_to_date` FROM `orders_item` WHERE MONTH(`order_to_date`) = MONTH(NOW()) AND YEAR(`order_to_date`) = YEAR(NOW()) AND `order_user_id`='" . $_SESSION['id'] . "' GROUP BY `order_id`");
                $sum_0   = 0.00;
                $sum_dop = 0.00;
                while ($_ROW_1 = mysqli_fetch_array($_QRY_1)) {
                    if ($_ROW_1['order_to_date'] < $LIMITUP_DATE) {
                        if ($_ROW_1['sum'] > $limit_old) {
                            $sum_0 += $limit_old;
                            $sum_dop += $_ROW_1['sum'] - $limit_old;
                        } else {
                            $sum_0 += $_ROW_1['sum'];
                            $sum_dop += 0.00;
                        }
                ...
                if ($_QRY_1 && ! empty($sum_0) && ! empty($sum_dop)) {
                    $output .= '<div class="d-flex flex-column align-items-center w-50">';
                    $output .= '<div class="digit text-info" data-toggle="popover" data-content="Сумма на текущий месяц, которая будет покрыта компанией из расчета лимита в ' . $limit . ' р. (до ' . date("d.m.Y", strtotime($LIMITUP_DATE)) . ' лимит был ' . $limit_old . ' р.)">' . number_format($sum_0, 0, ".", " ") . '</div>';
