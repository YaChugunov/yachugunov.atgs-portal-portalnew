    ...
    if (checkUserAuthorization($_SESSION['login'], $_SESSION['password']) == -1) {
        ...
        if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ! empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
            if ((checkServiceAccess('allservices') == 1 && checkServiceAccess('portalnew') == 1 && checkUserRestrictions($_SESSION['id'], 'portalnew', 2, 0) == 1) or (checkServiceAccess('allservices') == 1 && checkServiceAccess('portalnew') == 0 && checkIsItSuperadmin($_SESSION['id']) == 1) or (checkServiceAccess('allservices') == 0 && checkIsItSuperadmin($_SESSION['id']) == 1)) {
                ...
                $_QRY_NewPersons = mysqlQuery("
                SELECT kodwokerkart, wokerendname, wokerfistname, wokersecondname 
                FROM hr_docwokerkart 
                WHERE 
                kodwokerkart IN (SELECT kodwoker FROM hr_docworkerlaborcontr WHERE datelaborcontract > (NOW() - INTERVAL 7 DAY) AND showonmain='1' ORDER BY datelaborcontract DESC) 
                ");
                // $_QRY_NewPersons = mysqlQuery("
                // SELECT kodwokerkart, wokerendname, wokerfistname, wokersecondname 
                // FROM hr_docwokerkart 
                ...
                // kodwokerkart IN (SELECT kodwoker FROM hr_docworkerlaborcontr WHERE datelaborcontract > (NOW() - INTERVAL 7 DAY) AND showonmain='1' ORDER BY datelaborcontract DESC) 
                // OR 
                // kodwokerkart IN (SELECT kodwokerkart FROM hr_docwokerplace WHERE dateworkplace > (NOW() - INTERVAL 7 DAY) AND showonmain='1' ORDER BY dateworkplace DESC)
                // ");
                $personsIn       = "";
                $i               = 0;
                $namepodr        = "";
                ...
                while ($_ROW_NewPersons = mysqli_fetch_assoc($_QRY_NewPersons)) {
                    ...
                    SELECT kodwoker, koddoljprof, kodofficework, kodstructpodr, kodrazdidmain, kodrazdid, namedoljtmp FROM hr_docwokerproftmp WHERE kodwoker='{$_ROW_NewPersons['kodwokerkart']}'
                    "));

                    $_QRY_NewInfo3 = mysqli_fetch_assoc(mysqlQuery("
                    SELECT nameofficecity as city FROM hr_docworkerofficeplace WHERE kodofficeplace='{$_QRY_NewInfo2['kodofficework']}'
                    "));

                    if (! empty($_QRY_NewInfo2['kodrazdidmain'])) {
                        $_QRY_NewInfo5 = mysqli_fetch_assoc(mysqlQuery("
                    SELECT namerazdshot as nameshort, namerazdfull as namefull FROM hr_sporgschema_mains WHERE kodrazdid ='{$_QRY_NewInfo2['kodrazdidmain']}'
                    "));
                        $namepodrmain = ! empty($_QRY_NewInfo5['namefull']) ? $_QRY_NewInfo5['namefull'] : '';
                    }

                    if (! empty($_QRY_NewInfo2['kodrazdid'])) {
                        $_QRY_NewInfo4 = mysqli_fetch_assoc(mysqlQuery("
                    SELECT namerazdshot as nameshort, namerazdfull as namefull FROM hr_sporgschema WHERE kodrazdid ='{$_QRY_NewInfo2['kodrazdid']}'
                    "));
                        $namepodr = ! empty($_QRY_NewInfo4['nameshort']) ? $_QRY_NewInfo4['nameshort'] : '';
                    }

                    // if (empty($namepodr) && ! empty($_QRY_NewInfo2['kodstructpodr'])) {
                    //     $_QRY_NewInfo4 = mysqli_fetch_assoc(mysqlQuery("
                    //     SELECT shortname2 as nameshort FROM ism_spstructpodr WHERE kodstructpodr ='{$_QRY_NewInfo2['kodstructpodr']}'
                    //     "));
                    //     $namepodr = ! empty($_QRY_NewInfo4['nameshort']) ? $_QRY_NewInfo4['nameshort'] : '';
                    // }
                    ...
                    $outputArr['in'][$i]['dolj']     = ! empty($_QRY_NewInfo2['namedoljtmp']) ? $_QRY_NewInfo2['namedoljtmp'] : "";
                    $i++;
                }
                //
                ...
                while ($_ROW_OutPersons = mysqli_fetch_assoc($_QRY_OutPersons)) {
                    $_QRY_OutInfo1 = mysqli_fetch_assoc(mysqlQuery("SELECT numberdocworkend as numdoc, dateworkend as datedoc1, datedocworkend as datedoc2 FROM hr_docworkerworkend WHERE kodwoker='{$_ROW_OutPersons['kodwokerkart']}'"));

                    $_QRY_NewInfo2 = mysqli_fetch_assoc(mysqlQuery("
                    SELECT kodwoker, koddoljprof, kodofficework, kodstructpodr, kodrazdid, namedoljtmp FROM hr_docwokerproftmp WHERE kodwoker='{$_ROW_OutPersons['kodwokerkart']}'
                    "));
                    ...
                    $outputArr['out'][$i]['date']   = ! empty($_QRY_OutInfo1['datedoc1']) ? date('d.m.Y', strtotime($_QRY_OutInfo1['datedoc1'])) : "";
