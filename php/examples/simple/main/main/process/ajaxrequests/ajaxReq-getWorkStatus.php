    ...
    if (checkUserAuthorization($_SESSION['login'], $_SESSION['password']) == -1) {
        ...
        if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
            ...
            if ((checkServiceAccess('allservices') == 1 && checkServiceAccess('portalnew') == 1 && checkUserRestrictions($_SESSION['id'], 'portalnew', 2, 0) == 1) or (checkServiceAccess('allservices') == 1 && checkServiceAccess('portalnew') == 0 && checkIsItSuperadmin($_SESSION['id']) == 1) or (checkServiceAccess('allservices') == 0 && checkIsItSuperadmin($_SESSION['id']) == 1)) {
                ...
                $_QRY = mysqlQuery_atgsdinner("SELECT * FROM portal_absenceReportLog WHERE state='1' AND reportType NOT IN (9) ORDER BY timestamp DESC");
                $num = 0;
                if (mysqli_num_rows($_QRY) > 0) {
                    while ($_ROW = mysqli_fetch_array($_QRY)) {
                        ...
                        $reportName = !empty($_ROW['reportName']) ? $_ROW['reportName'] : "";
                        $reportStatus = !empty($_ROW['reportStatus']) ? $_ROW['reportStatus'] : "";
                        $reportType = !empty($_ROW['reportType']) ? $_ROW['reportType'] : "";
                        $reportOption = !empty($_ROW['reportOption']) ? $_ROW['reportOption'] : "";

                        $comment = !empty($_ROW['comment']) ? '<p class="mb-0"><i class="fa-solid fa-comment mr-2"></i>' . $_ROW['comment'] . '</p>' : "";
                        $controlEl = ($userID == $_SESSION['id']) ? '<div class="ml-auto d-flex"><div class="control-btn text-dark mx-1"><i class="fa-solid fa-pen-to-square fa-sm"></i></div></div>' : '';
                        ...
                        case 3:
                            ...
                            break;
                        case 4:
                            $reportIcon = '<i class="fa-solid fa-person-digging fa-lg"></i>';
                            break;
                        case 5:
                            $reportIcon = '<i class="fa-solid fa-house-laptop fa-lg" style="color:#0079fd;"></i>';
                            break;
                        default:
                            $reportIcon = '<i class="fa-solid fa-calendar-day fa-lg"></i>';
                        }
                        if (!empty($useLightTheme) && $useLightTheme == '1') {
                            $output .= '<div class="media p-2 mb-1 reportStatus" data-reportID="' . $reportID . '"><div class="rounded-circle mr-3">' . $reportIcon . '</div><div class="media-body d-flex flex-column"><div class="d-flex flex-row"><h5 class="mt-0 mb-1">' . $firstname . ' ' . $lastname . '</h5>' . $controlEl . '</div><p class="mb-0 text-dark">' . $reportName . '&nbsp;&bull;&nbsp;' . $reportStatus . '</p>' . $comment . '</div></div>';
                        } else {
                            $output .= '<div class="media p-2 mb-1 reportStatus" data-reportID="' . $reportID . '"><div class="rounded-circle mr-3">' . $reportIcon . '</div><div class="media-body d-flex flex-column"><div class="d-flex flex-row"><h5 class="mt-0 mb-1">' . $firstname . ' ' . $lastname . '</h5>' . $controlEl . '</div><p class="mb-0 text-white">' . $reportName . '&nbsp;&bull;&nbsp;' . $reportStatus . '</p>' . $comment . '</div></div>';
                        }
                        $num++;
                    }
                } else {
